...
function renderHomePage({ user, tasks, message, error, totalVolunteerCount }) {
  ...
  const authLinks = user
    ? `<form method="post" action="/logout" style="display:inline;"><button type="submit" style="background:none;border:none;color:blue;cursor:pointer;text-decoration:underline;padding:0;">Sign out</button></form>`
  ...
  const body = `
    ...
    <p>${authLinks} | <a href="/subscriptions">Find my subscriptions</a></p>
    <section>
      <h2>Active Tasks</h2>
      ${tasksHtml}
    </section>
